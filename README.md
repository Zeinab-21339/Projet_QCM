...
**The Legacy QCM** est une application web qui permet à des utilisateurs inscrits de passer un QCM de 10 questions tirées aléatoirement parmi une base d'environ 100 questions. À la fin du test, l'utilisateur reçoit une note sur 20 et peut consulter ses erreurs ainsi que son historique de tentatives.

Un **mode administrateur** permet de gérer les questions et les utilisateurs (ajout, modification, suppression, blocage).

L'application met également l'accent sur **la lutte contre la triche** : passage du QCM en plein écran obligatoire, détection de changement d'onglet, désactivation du clic droit et des raccourcis sensibles, minuterie globale.
...
- ✅ Tirage aléatoire de **10 questions** parmi ~100 questions en base
...
- ✅ **Historique** complet des tentatives + moyenne générale

### 🛡️ Mécanismes anti-triche

- 🖥️ **Plein écran obligatoire** pendant le QCM
- 👀 **Détection de sortie de plein écran** et de changement d'onglet
- ⚠️ Système d'**avertissements** (2 max) avant invalidation
- ⏱️ **Minuterie** de 10 minutes maximum
- 🚫 Désactivation du clic droit, des raccourcis (Ctrl+C, Ctrl+V, F12…) et de la sélection de texte
...
| **BDD**      | MySQL / MariaDB (via PDO)                   |
| **Serveur**  | Apache (XAMPP / WAMP / LAMP)                |

> ⚠️ Conformément au cahier des charges, **aucun framework PHP (Laravel, Symfony…) ni CMS** n'a été utilisé.
...
- **Apache** (via XAMPP, WAMP, MAMP ou LAMP)
- Un navigateur récent (Chrome, Firefox, Edge, Safari)

### Étapes

**1. Cloner ou copier le projet** dans le dossier web de votre serveur local :

```bash
# Exemple avec XAMPP sous Windows
C:\xampp\htdocs\Projet_QCM_final

# Exemple avec LAMP sous Linux
...
Le dump SQL contient déjà quelques comptes pour tester :
...
> ⚠️ **Important** : pour les nouveaux comptes créés via le formulaire d'inscription, les mots de passe sont **hachés via `password_hash()`** (bcrypt). Les comptes de démo ci-dessus sont en clair uniquement pour la phase de test.
...
4. Répondre aux 10 questions (timer 10 min)
5. **Valider** → page de résultats avec note /20 + correction
...
- Onglet **Questions** : ajouter / modifier / supprimer
- Onglet **Utilisateurs** : bloquer / débloquer / supprimer
...
```
utilisateurs                tentatives                    reponses
─────────────               ──────────                    ────────
id (PK)                ←─── utilisateur_id (FK)
nom                         id (PK)                ←───   tentative_id (FK)
prenom                      score                         id (PK)
email (UNIQUE)              date                          question_id (FK)  ───→ questions
mot_de_passe                                              reponse_utilisateur          id (PK)
role (user/admin)                                         correcte                     question
bloque                                                                                 reponse1..4
                                                                                       bonne_reponse
```
...
Les **contraintes de clés étrangères** assurent l'intégrité référentielle (`ON DELETE CASCADE`).
...
- 🛡️ **Protection contre l'injection SQL** : toutes les requêtes utilisent des **requêtes préparées PDO** avec paramètres liés
- 🍪 **Gestion sécurisée des sessions** PHP (`session_start()`, vérification de `$_SESSION['user_id']`)
- 🚪 **Middleware d'authentification** (`auth_check.php`, `auth_admin.php`) pour restreindre l'accès aux pages protégées
- ✉️ **Validation des emails** côté serveur via `filter_var()` + contrainte d'unicité en base
...
Pour toute question relative au projet, contactez l'équipe **The Legacy House** via l'établissement.
